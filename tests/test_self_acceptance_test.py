...
@pytest.mark.skip  # Set 10 seconds delay for S2 in docker-compose
@pytest.mark.integration
async def test_self_acceptance_iteration_2(
    ...
    start_worker(
        "worker2"
    )  # Set 10 seconds delay for S2 in docker-compose, otherwise this test would fail.
    ...
    assert (
        len(worker2_messages.messages) < 4
    )  # S2 is behind - this would fail if delay is not set
